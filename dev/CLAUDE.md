...
**Last updated: July 3, 2026** — A fifth bug from the same weekly-load investigation thread: **"Due Today" silently dropped threads whose parent had no date of its own.** Root cause: the Today/Overdue/Week buckets in `renderTodayView()`/`renderDailyView()`/the third near-identical view all suppress ANY thread with an active `parent_uuid` (nesting it under the parent's child-summary badge instead) — unconditionally, regardless of whether the parent itself ever appears in any bucket. Most parent/project threads have no `next_action_date` of their own, so a child due today would vanish from every view with nothing to nest it under. The child-summary badge made it worse: it only counted overdue children, never due-today ones, so even a visible parent wouldn't surface the urgency. Fixed by extracting the suppression rule (previously copy-pasted 3x, root of at least 3 bugs this session) into one shared `OAD.computeSuppressedChildUUIDs()` in engine.js — a child is now never suppressed if it's overdue or due today, only for lower-urgency dates. Removed the now-dead overdue-count badge (any suppressed child can no longer be overdue/due-today by construction). Four new unit tests cover the rule directly. Also this session: Focus Now date-scoping + celebrate-when-empty state, the reopen/closure-wizard misfire fix, "This Week's Load" composite score (configurable thresholds, no magic numbers), time-of-day scheduling, Runway Risk convergence check, Quick Add capture + Inbox nav, and the pressure propagation fix (199 tests passing).
